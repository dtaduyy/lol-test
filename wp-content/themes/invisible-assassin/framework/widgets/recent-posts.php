...
class invisible_assassin_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'invisible_assassin_rp', // Base ID
			__('Invisible Assassin Recent Posts with Thumbnails', 'invisible_assassin'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'invisible_assassin' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		$title = __('Latest Posts','invisible_assassin');
		$no_of_posts = 5;
	endif;
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'invisible_assassin' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'invisible_assassin' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','invisible_assassin' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'invisible_assassin' ); ?></label>
...
add_action( 'widgets_init', 'register_invisible_assassin_widget' );  
function register_invisible_assassin_widget() {  
    register_widget( 'invisible_assassin_Recent_Posts' );  
}  
